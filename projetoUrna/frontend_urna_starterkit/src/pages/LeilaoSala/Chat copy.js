...
const Chat = () => {
  ...
  */
  ...
  function onCountDownReceived(payload) {
    ...
    if(message == 0){
      if(currentFecthProducts == numberOfProducts-1 ){
        setManager('final');
      }
      setManager('second');
    }
  ...
  const handleCountdown2 = async () => {
    const response = await get(`/api/countdown/startcountdown`);
  ...
  const handleCountdown = async () => {
    stompClient.send('/app/startcountdown', {}, JSON.stringify({uuid: uuid,manager: manager}));
    setManager('pending')
    console.log('givent')
  }
              ...
              <Card>
                <CardBody>
                  ...
                  <div>Dinheiro em caixa: (R$) {money} </div>
                   ...
                   { lance !== 0 ?
                      <div className={styles.lanceDado}>Lance dado de : (R$) {lance} </div> : null
                   }
